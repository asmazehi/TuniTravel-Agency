...
bool Offre::ajouter()
{
    ...
    query.prepare("INSERT INTO offre (id, prix, contenu,nbredeplace,dated) "
...
{
    QSqlQuery query;
    query.prepare("UPDATE offre set prix=:prix,contenu=:contenu,nbredeplace=:nbredeplace,dated=:dated where id=:id");
    query.bindValue(":id", id);
    query.bindValue(":prix", prix);
    query.bindValue(":contenu", contenu);
    query.bindValue(":nbredeplace", nbredeplace);
...
QSqlQueryModel * Offre::trin()

  {
      ...
      model->setQuery("SELECT * FROM OFFRE ORDER BY nbredeplace");
...
void recherche(QTableView * table, QString);
...

 {
     ...
     QSqlQuery *query=new QSqlQuery;
         ...
         query->prepare("select * from OFFRE where ID like '%"+rech+"%' or CONTENU like '%"+rech+"%' ;");
 ...
 model->setQuery("select * from OFFRE where DATED like '%"+x+"%'");
 model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
 model->setHeaderData(1, Qt::Horizontal, QObject::tr("PRIX"));
 model->setHeaderData(2, Qt::Horizontal, QObject::tr("CONTENU"));
 model->setHeaderData(3, Qt::Horizontal, QObject::tr("NBREDEPLACE"));
