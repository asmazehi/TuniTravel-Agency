    ...
    ui(new Ui::Dialog)
{
   ...
   // ui->Lreference->setValidator( new QIntValidator(0, 99999999, this));
    ...
    QSqlQuery q,q1,q2,q3,q4,q5;
        ...
        q.exec("Select * from AV_FOURNITURE");
        int tot=0;
        while (q.next())
            tot++;
       qDebug() << "total =" << tot;
        q1.prepare("Select * from AV_FOURNITURE where etat = :nom ");
     ...
     qDebug() << "pourcentage don =" << tot_don;
        ...
        q2.prepare("Select * from AV_FOURNITURE where etat = :nom ");
        ...
        if(file.open(QIODevice::ReadOnly))
            {
             ...
             QSqlQuery query;
             query.prepare("INSERT INTO AV_FOURNITURE(IMAGE)" "VALUES (:IMAGE)");
...
void Dialog::on_pushButton_2_clicked()
{
    ...
    QSqlQuery query;
    query.prepare("update  AV_FOURNITURE set date_reparation = to_date(:datee,'dd/MM/yyyy') where reference = :id");
...
{
    QSqlQuery q;
    q.prepare("update AV_FOURNITURE set etat = :etat WHERE date_reparation < sysdate");
